...
- **`lib/`** — the pure, testable core. Key separation: orchestrators do I/O, lib functions are pure and injected with config.
  ...
  - `trust.ts` — `decideResume()`: bot comments ignored, trusted associations resume, untrusted humans hold until maintainer approval. `isApproval()` decides whether an epic comment approves a proposed breakdown; anchored to the first non-quoted line and negation-vetoed, because a substring match reads "I don't approve" as consent.
  - `decompose.ts` / `plan-block.ts` — epic decomposition mode and the propose→approve loop. `parseDecomposeMode()` resolves dispatched mode (an `approve` comment) → the epic body's "Decomposition intent" dropdown → `FACTORY_DECOMPOSE_MODE` → baked `propose`, never guessing `auto`. `plan-block.ts` round-trips the plan through a base64 HTML-comment block (base64 because a subtask body containing `-->` would close the comment) so approval replays the reviewed plan with no model call — re-prompting the decomposer does not return the same split twice.
